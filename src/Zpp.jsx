...
const PROJECTS = [
    {
        id: 1,
        title: "StageConnect",
        desc: "A fullstack internship management platform connecting students, companies, and academic institutions. Features role-based access, application tracking, and an admin panel. Built with Java EE and MySQL.",
...
function Icon({ name, size = 18, color = "currentColor", strokeWidth = 1.7 }) {
    ...
    return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
            ...
            {subpaths.map((subpath, i) => <path key={i} d={subpath.trim()} />)}
        </svg>
    );
}
            ...
            >
                <div style={{ width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {skill.fallback ? (
                        ...
                        <StackIcon name={skill.stackIcon} variant={dark ? "dark" : "light"} style={{ width: "100%", height: "100%" }} />
                    )}
                </div>
            </motion.div>
...
function WordFade({ text, delay = 0, style = {} }) {
    const words = text.split(" ");
    const container = { hidden: {}, visible: { transition: { staggerChildren: 0.06, delayChildren: delay } } };
    const word = { hidden: { opacity: 0, y: 14, filter: "blur(6px)" }, visible: { opacity: 1, y: 0, filter: "blur(0px)", transition: { duration: 0.55, ease: [0.22, 1, 0.36, 1] } } };
    const ref = useRef(null);
    const inView = useInView(ref, { once: true, margin: "-40px" });
    return (
        <motion.span ref={ref} variants={container} initial="hidden" animate={inView ? "visible" : "hidden"} style={{ display: "inline", ...style }}>
            {words.map((w, i) => (
        ...
        <span ref={ref} style={style}>
            ...
            {displayed.length < text.length && (
                <motion.span animate={{ opacity: [1, 0] }} transition={{ repeat: Infinity, duration: 0.7 }}
                    style={{ display: "inline-block", width: 2, height: "1em", background: "currentColor", verticalAlign: "middle", marginLeft: 2 }} />
            )}
        </span>
    );
}
...
function HeroName({ t }) {
    ...
    const charAnim = (i, isLine2 = false) => ({
        ...
        animate: { y: "0%", opacity: 1, filter: "blur(0px)", rotateX: 0, skewX: 0, transition: { duration: 0.85, delay: 0.25 + i * 0.055, ease: [0.22, 1, 0.36, 1] } },
    });
    ...
    const nameStyle = {
        ...
        marginBottom: 32, perspective: "1200px", userSelect: "none",
        wordBreak: "break-word", width: "100%", maxWidth: "100%", boxSizing: "border-box",
    };
    ...
    return (
        <div style={nameStyle}>
            <div style={{ display: "flex", overflow: "hidden", paddingBottom: "0.08em" }}>
                {line1.map((ch, i) => (
                    <motion.span key={i} style={{ display: "inline-block", color: t.ink, cursor: "default" }}
                        {...charAnim(i)}
            ...
            <div style={{ display: "flex", overflow: "hidden", paddingBottom: "0.08em", position: "relative" }}>
                {line2.map((ch, i) => (
                    <motion.span key={i}
                        style={{ display: "inline-block", fontStyle: "italic", color: t.accent, cursor: "default", position: "relative" }}
                ...
                {shimmer && (
                    <motion.div
                        initial={{ x: "-110%", opacity: 0.7 }} animate={{ x: "110%", opacity: 0 }} transition={{ duration: 0.9, ease: [0.4, 0, 0.2, 1] }}
                        style={{ position: "absolute", inset: 0, background: `linear-gradient(105deg, transparent 30%, ${t.accent}60 50%, transparent 70%)`, pointerEvents: "none", borderRadius: 4 }} />
                )}
            </div>
        </div>
    );
        ...
        <section id="hero" style={{ minHeight: "100svh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "clamp(90px,12vw,120px) clamp(20px,5vw,60px) clamp(32px,4vw,44px)", position: "relative", overflow: "hidden" }}>
            ...
            <motion.div style={{ y, opacity, maxWidth: 1200, width: "100%", boxSizing: "border-box", overflow: "hidden" }}>
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.1 }}
                    ...
                    <motion.span initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.6, delay: 0.2 }}
                        style={{ display: "inline-block", width: 28, height: 1, background: t.accent, flexShrink: 0, transformOrigin: "left" }} />
                ...
                <div className="hero-layout">
                    ...
                    <motion.div className="hero-right"
                        ...
                        <motion.div variants={{ hidden: { opacity: 0, x: 48 }, visible: { opacity: 1, x: 0, transition: { duration: 0.7, ease: [0.22, 1, 0.36, 1] } } }} style={{ marginBottom: 16 }}>
                        ...
                        <motion.p variants={{ hidden: { opacity: 0, x: 48 }, visible: { opacity: 1, x: 0, transition: { duration: 0.7, ease: [0.22, 1, 0.36, 1] } } }}
        ...
        <section id="about" ref={ref} className="section-padded" style={{ maxWidth: 1260, margin: "0 auto" }}>
            <div className="about-grid">
                <SlideIn direction={-1} delay={0.1}>
                    <motion.div className="about-photo-wrap" style={{ position: "relative" }} onHoverStart={triggerGloss} onTap={triggerGloss}>
                ...
                <div style={{ minWidth: 0 }}>
                    ...
                    <RevealText delay={0.2}>
                        <div className="about-pills">
                            {[
                                ["MapPin", "Algiers, Algeria"],
                                ["GraduationCap", "INSFP Rahmania"],
                                ["Briefcase", "Open to Work"],
                                ["Building", "Apprentice @ Sonelgaz"],
                            ].map(([icon, label]) => (
...
function SkillMarquee({ t }) {
    ...
    return (
        <div style={{ display: "flex", gap: "clamp(32px,4vw,52px)", overflow: "hidden", WebkitMaskImage: "linear-gradient(90deg,transparent,black 10%,black 90%,transparent)" }}>
            <motion.div style={{ display: "flex", gap: "clamp(32px,4vw,52px)", flexShrink: 0, whiteSpace: "nowrap" }}
            ...
            <div style={{ maxWidth: 1260, margin: "0 auto" }}>
                ...
                <RevealText>
                    <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(28px,4vw,52px)", fontWeight: 700, color: t.ink, marginBottom: 16 }}>
                        My technical <em style={{ color: t.accent, fontStyle: "italic" }}>stack.</em>
                    </h2>
                </RevealText>
                <RevealText delay={0.05}>
                    <p style={{ fontFamily: "Georgia, serif", fontSize: 15, color: t.inkSoft, marginBottom: 64, maxWidth: 520, lineHeight: 1.8 }}>
                        Technologies I've worked with across real projects and professional training.
                    </p>
                </RevealText>
    ...
    return (
        <section id="projects" className="section-padded">
            <div style={{ maxWidth: 1260, margin: "0 auto" }}>
                ...
                <RevealText>
                    <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(28px,4vw,52px)", fontWeight: 700, color: t.ink, marginBottom: 16 }}>
                        Selected <em style={{ color: t.accent, fontStyle: "italic" }}>Work.</em>
                    </h2>
                </RevealText>
    ...
    const cards = [
        {
            ...
            date: "Oct 2024 – Present · Algiers",
            role: "Software Development Apprentice",
            org: "One of Algeria's Largest State Enterprises",
            desc: "Currently apprenticing at one of Algeria's largest state enterprises, where I've spent over a year in a real enterprise environment — observing professional workflows, building projects as part of my training, and developing a practical understanding of software in large-scale organizations.",
        },
        {
            side: 1, delay: 0.35,
            ...
            org: "INSFP Rahmania · Algiers, Algeria",
            desc: "Specializing in fullstack web development — from algorithms and data structures to production-grade applications.",
        },
    ];
        ...
        <section id="experience" ref={ref} className="section-padded" style={{ background: `linear-gradient(to bottom, transparent, ${t.bg2}55, transparent)` }}>
            <div style={{ maxWidth: 1260, margin: "0 auto" }}>
                ...
                <div className="experience-grid">
                    {cards.map(({ side, delay, label, icon, date, role, org, desc }) => (
    ...
    return (
        <section id="contact" ref={ref} className="section-padded-bottom">
            <div style={{ maxWidth: 1260, margin: "0 auto" }}>
                ...
                <motion.div initial="hidden" animate={inView ? "visible" : "hidden"} variants={stagger} className="contact-grid">
                    {contacts.map((c, i) => (
...
export default function App() {
    ...
    return (
        ...
        *, *::before, *::after { box-sizing: border-box; max-width: 100%; }
        ...
        .hero-layout { display: flex; flex-direction: column; width: 100%; }
        .hero-left { width: 100%; min-width: 0; }
        .hero-right { display: none !important; }
        .hero-cta-mobile { display: flex !important; }
        .hero-role-mobile { display: flex !important; }

        @media (min-width: 900px) {
          .hero-layout { flex-direction: row; align-items: center; gap: clamp(32px, 4vw, 60px); }
          .hero-left { flex: 1; min-width: 0; }
          .hero-right { display: flex !important; flex-shrink: 0; width: clamp(260px, 28vw, 360px); }
          .hero-cta-mobile { display: none !important; }
          .hero-role-mobile { display: none !important; }
        }

        .about-grid { display: grid; grid-template-columns: 1fr; gap: clamp(32px, 6vw, 80px); align-items: center; }
        @media (min-width: 640px) {
          .about-grid { grid-template-columns: repeat(auto-fit, minmax(min(100%, 300px), 1fr)); }
        }

        .about-photo-wrap { max-width: 200px; margin: 0 auto; width: 100%; }
        .about-photo-inner { aspect-ratio: 1 / 1; border-radius: 50%; overflow: hidden; }
        .about-photo-border { display: none; }
        @media (min-width: 640px) {
          .about-photo-wrap { max-width: 100%; margin: 0; }
          .about-photo-inner { aspect-ratio: 3 / 4; border-radius: 6px; }
          .about-photo-border { display: block; }
        }
        ...
        .about-photo-gloss.gloss-run { animation: glossSweep 0.9s ease-in-out forwards; }

        .about-pills { display: flex; gap: 12px; margin-top: 32px; flex-wrap: wrap; }
        @media (max-width: 480px) { .about-pills { gap: 8px 16px; } }

        .skill-grid { display: flex; flex-wrap: wrap; gap: 18px; }
        @media (max-width: 480px) { .skill-grid { gap: 14px; } }

        .soft-skills-grid { display: grid; grid-template-columns: 1fr; gap: 18px; }
        @media (min-width: 480px) {
          .soft-skills-grid { grid-template-columns: repeat(auto-fill, minmax(min(100%, 278px), 1fr)); }
        }

        .projects-grid { display: grid; grid-template-columns: 1fr; gap: 24px; }
        @media (min-width: 480px) {
          .projects-grid { grid-template-columns: repeat(auto-fill, minmax(min(100%, 300px), 1fr)); }
        }

        .experience-grid { display: grid; grid-template-columns: 1fr; gap: clamp(40px, 5vw, 60px); }
        @media (min-width: 640px) {
          .experience-grid { grid-template-columns: repeat(auto-fit, minmax(min(100%, 280px), 1fr)); }
        }

        .contact-grid { display: grid; grid-template-columns: 1fr; gap: 14px; margin-bottom: 80px; width: 100%; box-sizing: border-box; }
        @media (min-width: 560px) {
          .contact-grid { grid-template-columns: repeat(auto-fill, minmax(min(100%, 240px), 1fr)); }
        }

        .contact-footer { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; }

        .section-padded { padding: clamp(72px, 10vw, 120px) clamp(16px, 5vw, 60px); }
        .section-padded-bottom { padding: clamp(72px, 10vw, 120px) clamp(16px, 5vw, 60px) clamp(48px, 6vw, 80px); }
        @media (max-width: 380px) {
          .section-padded, .section-padded-bottom { padding-left: 14px; padding-right: 14px; }
        }
      `}</style>
